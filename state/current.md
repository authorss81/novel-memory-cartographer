...
**An independent review has now run on this batch, three times, and its findings have been applied.** `logs/phase-002-batch-plan.review.log` records a full read of Chapters 1–10. The first pass found that the batch's spine held but that the **state layer had drifted ahead of the prose**; twelve repairs were made and the state files reconciled to the corrected prose, logged as **CR-015**. A second independent pass then re-read the repaired batch and found two **blockers** and five **majors** that the first pass had missed — the most serious being that Chapter 8 had Jo Lask calling the interval from the tram side while he was trapped inside the loop, and that Chapter 3 and Chapter 6 gave the crossing point as 62 m and 410 m from the platform in the same sworn chain. Those and the rest are now fixed and logged as **CR-016**, with a verification pass confirming no blockers remain. No premise, rule, cost, antagonist, or calendar date moved in either pass. A **third** pass, in `logs/batch-0001.review.log`, then found **two blockers that the first two passes had created or preserved and propagated into five files each**: Chapter 1's forensic deduction asserted a chronology no physical fold can support, and `chapter-0010.md` had Milo spend his dead mother's stairwell check to buy a favour, undoing Chapter 2:99, Chapter 3:127–131, Chapter 8:89 and the volume's disclosure discipline. It also found three majors — the Owen Fitch contradiction across four chapters, which CR-016 had papered over with a **false** rationale now annotated in place; an unflagged fifty-five-minute hole inside Chapter 8; and five files asserting an event the prose did not contain — plus a set of number slips and two mechanical prose tics. All are fixed and logged as **CR-017**. No chapter needed re-cutting; four passages were re-read and eleven state lines rewritten. The fourth and fifth were the Chapter 1 clue, now a wet transfer onto a shut damped sheet rather than a chronology, and **Owen's initials, which Mara put on the 5th-of-Sazur inspection instrument and which the next batch must not treat as clean paper.** No premise, rule, cost, antagonist, or calendar date moved in any pass. The review gate itself is still broken as a mechanism — see pending controller action 2 — but this phase is no longer self-checked only.
...
- **5th:** Chapter 6, pre-dawn, Hall of Wards. This is a **Directions Bench variation**, not the hearing. The one-shift inspection authorised that morning names five people, and **Owen Fitch's initials are on it** — put there by Mara herself, because the protective finding required persons in temporary accommodation. He finds out in the Chapter 9 yard and makes the refusal line carry them.
- **6th:** Chapter 7 at sunrise (Milo calls the interval 7:00–7:09, Danner winds the gate at 7:09, Jo is in the loop by 7:12), Chapter 8 late morning (Pell's count runs from seven and expires as Jo comes out; Jo out at ten past eight, which is the end of Pell's hour; the amended sheet on the wire tray at 11:40).
